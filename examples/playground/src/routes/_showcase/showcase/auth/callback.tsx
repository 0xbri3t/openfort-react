...
function RouteComponent() {
  ...
  const { isSuccess, isError, error, provider, email } = useAuthCallback({
    onSuccess: (data) => {
      if (data.type === 'verifyEmail' && data.email) setEmail(data.email)
    },
  })
    ...
    <DialogLayout>
      ...
      {isSuccess && (
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">You have successfully authenticated with {email || provider}.</p>
          {provider === 'email' ? (
      ...
      {isError && (
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">There was an error authenticating with {provider}.</p>
          <InputMessage message={error?.message || 'An unknown error occurred.'} variant="error" show={!!error} />
        </div>
      )}
      {!isSuccess && !isError && (
        <div className="text-center">
