...
const formatValue = (val: any, valueType: string, _name: string) => {
  switch (valueType) {
    ...
    case 'object': {
      const cons = val.constructor?.name || 'Object'
      const keys = getObjectKeys(val)
      return `${cons} {${keys.length} ${keys.length === 1 ? 'property' : 'properties'}}`
    }
    case 'date':
      ...
      <div className="ml-[6.5px] border-l border-gray-200 dark:border-gray-600 pl-4 flex flex-col gap-0.5 pt-1">
        {keys.map((key, index) => {
          try {
            ...
            return (
              // biome-ignore lint/suspicious/noArrayIndexKey: allowed for simplicity
              <div key={`${key}-${index}`} className="mb-1 text-red-500 dark:text-red-400">
